...
class Dataset:
    ...
    @classmethod
    def load_builtin(cls, name='ml-100k', binary=False, pos_ne_divider=None):
        ...
        print('load_builtin'+str(binary)+str(pos_ne_divider))

        return cls.load_from_file(file_path=dataset.path, reader=reader, binary=binary, pos_ne_divider=pos_ne_divider)

    @classmethod
    def load_from_file(cls, file_path, reader, binary=False, pos_ne_divider=None):
        ...
        Args:
            ...
            reader(:obj:`Reader <surprise.reader.Reader>`): A reader to read
                the file.
        """
        print('load_from_file'+str(binary)+str(pos_ne_divider))

        return DatasetAutoFolds(ratings_file=file_path, reader=reader, binary=binary, pos_ne_divider=pos_ne_divider)

    @classmethod
    def load_from_folds(cls, folds_files, reader, binary=False, pos_ne_divider=None):
        ...
        return DatasetUserFolds(folds_files=folds_files, reader=reader, binary=binary, pos_ne_divider=pos_ne_divider)

    @classmethod
    def load_from_df(cls, df, reader, binary=False, pos_ne_divider=None):
        ...
        Args:
            ...
            reader(:obj:`Reader <surprise.reader.Reader>`): A reader to read
                ...
                specified.
        """
        print('load_from_df'+str(binary)+str(pos_ne_divider))

        return DatasetAutoFolds(reader=reader, df=df, binary=binary, pos_ne_divider=pos_ne_divider)
    ...
    def construct_trainset(self, raw_trainset):
        ...
        ur = defaultdict(list)
        ir = defaultdict(list)
        print('construct_trainset'+str(self.binary)+str(self.pos_ne_divider))
        ...
        for urid, irid, r, timestamp in raw_trainset:
            ...
            if self.binary:
                if self.pos_ne_divider is not None:
                    ur[uid].append((iid, 1 if r > self.pos_ne_divider else 0))
                    ir[iid].append((uid, 1 if r > self.pos_ne_divider else 0))
                else:
                    ur[uid].append((iid, 1))
                    ir[iid].append((uid, 1))
            else:
    ...
    def construct_testset(self, raw_testset):
        print('construct_testset'+str(self.binary)+str(self.pos_ne_divider))
        if self.binary:
            if self.pos_ne_divider is not None:
                return [(ruid, riid, 1 if r_ui_trans > self.pos_ne_divider else 0) for (ruid, riid, r_ui_trans, _) in raw_testset]
            else:
                return [(ruid, riid, 1 ) for (ruid, riid, r_ui_trans, _) in raw_testset]
        else:
...
class DatasetUserFolds(Dataset):
    ...
    def __init__(self, folds_files=None, reader=None, binary=False, pos_ne_divider=None):

        Dataset.__init__(self, reader)
        self.folds_files = folds_files
        self.binary = binary
        self.pos_ne_divider = pos_ne_divider
...
class DatasetAutoFolds(Dataset):
    ...
    def __init__(self, ratings_file=None, reader=None, df=None, binary=False, pos_ne_divider=None):
