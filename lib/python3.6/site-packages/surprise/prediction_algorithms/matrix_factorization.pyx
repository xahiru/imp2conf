...
class SVDppimp(AlgoBase):
    ...
    def __init__(self, n_factors=20, i_imp_factors=False, binary=False, n_epochs=20, init_mean=0, init_std_dev=.1,
                 lr_all=.007, reg_all=.02, lr_bu=None, lr_bi=None, lr_pu=None,
                 lr_qi=None, lr_yj=None, lr_kj=None, reg_bu=None, reg_bi=None, reg_pu=None,
                 reg_qi=None, reg_yj=None, reg_kj=None, random_state=None, verbose=False):
        ...
        self.reg_yj = reg_yj if reg_yj is not None else reg_all
        self.binary = binary
        self.i_imp_factors = i_imp_factors
        if self.i_imp_factors:
            self.reg_kj = reg_kj if reg_kj is not None else reg_all
            self.lr_kj = lr_kj if lr_kj is not None else lr_all
        self.random_state = random_state
        self.verbose = verbose
    ...
    def sgd(self, trainset):
        ...
        yj = rng.normal(self.init_mean, self.init_std_dev,
                        (trainset.n_items, self.n_factors))
        if self.i_imp_factors:
            kj = rng.normal(self.init_mean, self.init_std_dev,
                            (trainset.n_items, self.n_factors))
            i_impl_fdb = np.zeros(self.n_factors, np.double)
        u_impl_fdb = np.zeros(self.n_factors, np.double)
        ...
        for current_epoch in range(self.n_epochs):
            ...
            for u, i, r in trainset.all_ratings():
                ...
                # compute item implicit feedback
                #TODO if not using remove this
                if self.i_imp_factors:
                    Ui = [k for (k, _) in trainset.ir[i]]
                    sqrt_Ui = np.sqrt(len(Ui))
                    i_impl_fdb = np.zeros(self.n_factors, np.double) 

                    for k in Ui:
                        for f in range(self.n_factors):
                            i_impl_fdb[f] += kj[k, f] / sqrt_Ui
                ...
                for f in range(self.n_factors):
                    # dot += qi[i, f] * (pu[u, f] + u_impl_fdb[f])
                    if self.i_imp_factors:
                        dot += (qi[i, f] + i_impl_fdb[f]) * (pu[u, f] + u_impl_fdb[f])
                    else:
                        dot += qi[i, f] * (pu[u, f] + u_impl_fdb[f])
                if self.binary:
                    # 1 is used for implicit binary rating
                    err = 1 - (global_mean + bu[u] + bi[i] + dot)
                else:
                    err = r - (global_mean + bu[u] + bi[i] + dot)
                ...
                for f in range(self.n_factors):
                    ...
                    # pu[u, f] += lr_pu * (err * qif - reg_pu * puf)
                    if self.i_imp_factors:
                        pu[u, f] += lr_pu * (err * (qif + i_impl_fdb[f]) - reg_pu * puf)
                    else:
                        pu[u, f] += lr_pu * (err * qif - reg_pu * puf)
                    qi[i, f] += lr_qi * (err * (puf + u_impl_fdb[f]) -
                                         reg_qi * qif)
    ...
    def estimate(self, u, i):
        ...
        if self.trainset.knows_user(u) and self.trainset.knows_item(i):
            ...
            u_impl_feedback = (sum(self.yj[j] for (j, _)
                               in self.trainset.ur[u]) / np.sqrt(Iu))
            Ui = len(self.trainset.ir[i])
            i_imp_factors = (sum(self.kj[k]) for (k, _)
                                in self.trainset.ir[i]) / np.sqrt(Ui)
            est += np.dot(self.qi[i] + i_imp_factors, self.pu[u] + u_impl_feedback)
