...
class SVDppimp(AlgoBase):
    ...
    def sgd(self, trainset):
        ...
        self.mean_trust = 0
        for u, i, r in trainset.all_ratings():
            i_pops = len(trainset.ir[i])/trainset.n_items
            # u_pops = 1/len(trainset.ur[u])
            u_pops = len(trainset.ur[u])/trainset.n_users
            self.i_pops[i] = i_pops
            self.u_pops[u] = u_pops
            t = i_pops * u_pops
            self.mean_trust += t
            all_ratings.append((u,i,r,t))
        self.mean_trust = self.mean_trust/trainset.n_ratings
        ...
        for current_epoch in range(self.n_epochs):
            ...
            for u, i, r, t in all_ratings:
                ...
                # err = r - (global_mean + bu[u] + bi[i] + dot + t)
                err = r - (global_mean + bu[u] + bi[i] + dot)
    ...
    def estimate(self, u, i):
        ...
        if self.trainset.knows_user(u) and self.trainset.knows_item(i):
            ...
            u_impl_feedback = (sum(self.yj[j] for (j, _)
                               in self.trainset.ur[u]) / np.sqrt(Iu))
            est += np.dot(self.qi[i], self.pu[u] + u_impl_feedback)
            # t = 1/-np.log(self.i_pops[i] * self.u_pops[u])
            t = self.i_pops[i] * self.u_pops[u]
            # x = est * t
            if t > self.mean_trust:
                x = est + t
            else:
                x = est * t
            est += x
